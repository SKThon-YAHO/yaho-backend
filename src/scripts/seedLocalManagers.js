...
const PLACEHOLDER_PASSWORD = 'changeme_temp_password'; // 실제 가입 전까지 쓰는 더미 비밀번호
...
async function collectAllCodes() {
  ...
  while (true) {
    ...
    items.forEach((item) => {
      if (item.OPN_ATMY_GRP_CD) codes.add(item.OPN_ATMY_GRP_CD);
    });

    console.log(`코드 수집 중: ${pageNo * ROWS_PER_PAGE} / ${totalCount} (현재까지 ${codes.size}개 코드)`);
...
async function seedManagers(client, codes, hashedPassword) {
  ...
  const placeholders = codes
    .map((code, i) => {
      const base = i * 2;
      values.push(code, hashedPassword);
      return `($${base + 1}, $${base + 2})`;
    })
    .join(', ');

  const query = `
    INSERT INTO local_manager (local_code, password)
    VALUES ${placeholders}
...
async function main() {
  ...
  try {
    ...
    const hashedPassword = await bcrypt.hash(PLACEHOLDER_PASSWORD, 10);

    // 한 번에 너무 많이 넣지 않도록 500개씩 배치 처리
    const BATCH_SIZE = 500;
    for (let i = 0; i < codes.length; i += BATCH_SIZE) {
      const batch = codes.slice(i, i + BATCH_SIZE);
      await seedManagers(client, batch, hashedPassword);
      console.log(`local_manager 저장 진행: ${Math.min(i + BATCH_SIZE, codes.length)} / ${codes.length}`);
    }

    console.log('완료: local_manager에 미배정 코드 채우기 끝');
  } finally {
    client.release();
    await pool.end();
  }
