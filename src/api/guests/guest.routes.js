 ...
 * @req body { toilet_code: string, uuid: string(nullable) }
 * @res 200 { success: true, data: { uuid: string } }
 * @res 400 MISSING_REQUIRED_FIELDS
 */
router.post('/usage', guestController.LoggingUsage);

/**
 * @api-docgen
 * @tag Guest
 * @summary 설문 라우트 survey는 아무것도 체크 안 함 -> 0, 청결도만 체크 -> 1, 비품만 체크 -> 2, 전부 체크 -> 3 으로 세팅
 * @req body { toilet_code: string, survey: int, uuid: string }
 * @res 200 { success: true }
 * @res 400 MISSING_REQUIRED_FIELDS
 */
router.post('/survey', guestController.LoggingSurvey);
